...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        db_info_path = str(self.root_path) + '/' + sampler_cfg.DB_INFO_PATH[0]
        # remove it for nuscenes
        # for db_info_path in sampler_cfg.DB_INFO_PATH:
        #     db_info_path = self.root_path.resolve() / db_info_path
        #     print("="*100, "db_info_path", db_info_path, sep='\n')
    ...
    def put_boxes_on_road_planes(gt_boxes, road_planes, calib):
        ...
        Args:
            ...
            road_planes: [a, b, c, d] or None
            calib:

        Returns:
        """
        if road_planes is None:
            return gt_boxes, np.zeros(gt_boxes.shape[0])
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        # Store road_plane and calib temporarily if they exist
        road_plane = data_dict.get('road_plane', None)
        calib = data_dict.get('calib', None)
        
        if self.sampler_cfg.get('USE_ROAD_PLANE', False) and mv_height is None:
            sampled_gt_boxes, mv_height = self.put_boxes_on_road_planes(
                sampled_gt_boxes, road_plane, calib
            )
        ...
        if self.img_aug_type is not None:
            # Make sure road_plane is available for copy_paste_to_image
