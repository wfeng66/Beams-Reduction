...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None, current_fold=0):
        """
        Args:
            dataset_cfg: Dataset configuration
            class_names: List of class names
            training: Whether this is for training or testing
        ...
        # Handle root path with or without VERSION
        if root_path is not None:
            self.root_path = root_path
        else:
            if hasattr(self.dataset_cfg, 'VERSION'):
        ...
        self.num_folds = 5  # Total number of folds for cross-validation
    ...
    def get_fold_indices(self):
        ...
        fold_size = len(self) // self.num_folds
        
        # Create fold indices
        val_start = self.current_fold * fold_size
        val_end = (self.current_fold + 1) * fold_size if self.current_fold < self.num_folds - 1 else len(self)
        
        val_indices = all_indices[val_start:val_end]
        train_indices = np.concatenate([
            all_indices[:val_start],
            all_indices[val_end:]
        ])
        
        return train_indices if self.training else val_indices
    ...
    def __getitem__(self, index):
        ...
        actual_index = fold_indices[index]
        raise NotImplementedError
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                if key == 'calib':
                    # Keep calibration as a list to maintain individual calibration objects
                    ret[key] = val
