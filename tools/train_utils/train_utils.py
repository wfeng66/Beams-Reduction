...
def train_model(model, optimizer, train_loader, val_loader, model_func, lr_scheduler, optim_cfg,
    ...
    Args:
        ...
        cfg: Config object containing dataset and model configurations
    """
    ...
    eval_args = Namespace()
    eval_args.save_to_file = True
    eval_args.infer_time = False
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            # After training for this epoch, evaluate on validation set
            # if cfg.EVAL_INTERVAL and (cur_epoch + 1) % cfg.EVAL_INTERVAL == 0:
            val_metric = eval_utils.eval_one_epoch(
                cfg, eval_args, model, val_loader, epoch_id=cur_epoch, logger=logger, result_dir=ckpt_save_dir / 'eval'
            )

            # Choose your metric for best model tracking
            # Example: mean of three main classes
            metric_keys = [
                'Car_3d/moderate_R40',
                'Pedestrian_3d/moderate_R40',
                'Cyclist_3d/moderate_R40'
            ]
            current_score = np.mean([val_metric.get(k, 0) for k in metric_keys])

            if (best_metric is None) or (current_score > best_metric):
                best_metric = current_score
                best_epoch = cur_epoch
                # Save the best model
                best_model_path = ckpt_save_dir / 'best_model.pth'
                torch.save(model.state_dict(), str(best_model_path))
                logger.info(f"New best model saved at epoch {cur_epoch} with mean 3D AP {current_score:.4f}")
    ...
    if rank == 0:
        logger.info(f'Training finished. Best model was at epoch {best_epoch} with performance {best_metric:.4f}')
