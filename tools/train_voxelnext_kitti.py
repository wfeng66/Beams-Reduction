...
def log_metrics_to_file(result_dict, csv_file, args):
    ...
    # Get model name from cfg file
    model_name = Path(args.cfg_file).stem
    # Get dataset name from cfg file path
    dataset_name = 'NuScenesDataset' if 'nuscenes' in args.cfg_file.lower() else 'KITTIDataset'
    ...
    if 'nuscenes' in args.cfg_file.lower():
        ...
        for cls_name in classes:
            # Define the column name for the CSV - Using AP@0.5
            column_key = f'{cls_name}_AP@0.5'
            
            # Look up the actual key in result_dict
            result_key = f'{cls_name}_AP@0.5'
            
            if result_key in result_dict:
                # The values should already be in [0,1], convert to percentage
                metrics_to_save[column_key] = str(round(result_dict[result_key] * 100, 2))
            else:
                metrics_to_save[column_key] = '0.0'
    else:
        # For KITTI dataset, convert moderate_R40 to AP@0.5 format
        kitti_classes = ['Car', 'Pedestrian', 'Cyclist']
        for cls_name in kitti_classes:
            # Convert moderate_R40 to AP@0.5 format
            key = f'{cls_name}_3d/moderate_R40'
            if key in result_dict:
                metrics_to_save[f'{cls_name.lower()}_AP@0.5'] = str(round(result_dict[key] * 100, 2))
            else:
                metrics_to_save[f'{cls_name.lower()}_AP@0.5'] = '0.0'
...
def main():
    ...
    # Create dynamic filename based on model and dataset
    model_name = Path(args.cfg_file).stem
    dataset_name = 'nuscenes' if 'nuscenes' in args.cfg_file.lower() else 'kitti'
    csv_file = os.path.join(output_dir, f'{model_name}_{dataset_name}_metrics.csv')
